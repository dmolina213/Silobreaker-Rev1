...
def misp_check_for_previous_event(misp_instance, isight_alert):
    ...
    if isight_alert.Description:
        result = misp_instance.search(value=isight_alert.Description, type_attribute='email-src', category='Payload delivery')
...
def create_misp_event(misp_instance, isight_report_instance):
    ...
    PySilo_settings.logger.debug('create_misp_event 474:Created MISP event %s for iSight report %s', my_event, isight_report_instance.Id)
...
def process_isight_indicator(a_json):
    ...
    try:
        ...
        if this_misp_instance is False:
            ...
            PySilo_settings.logger.debug("583 process_isight_indicator: No MISP Instance found: ", this_misp_instance )     
